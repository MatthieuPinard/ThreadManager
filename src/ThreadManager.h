...
public:
	...
	*
	*  The contructor takes a Callback function as parameter, taking an unsigned int as argument and returning an unsigned int.
	*/
	template <class Fn>
	explicit ThreadManager(Fn&& fn) : DTOR_ThreadValuesSum(0), Callback(fn), ValueUniqueLock(std::unique_lock<AtomicLock>(ValueLock, std::defer_lock)) {	
	}
	/*!
